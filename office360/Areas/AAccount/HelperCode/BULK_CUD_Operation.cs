            ...
            using (var db = new FASEntities())
            {
                using (System.Data.Entity.DbContextTransaction dbTran = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        int? DB_OPERATION_STATUS = AAccount.HelperCode.Check_Duplicate_By_LINQ.IS_EXIST_ACFSM_CLASSFEESTRUCTURE_BY_PARAMETER(PostedData);
                        switch (DB_OPERATION_STATUS)
                        {
                            case (int?)Http_DB_Response.CODE_AUTHORIZED:
                                ...
                                var ACCM_ClassCurriculum = DBFStoredProcedure.ACFSM_ClassFeeStructure_Upsert(
                                                                PostedData.OperationType,
                                                                PostedData.GuID,
                                                                PostedData.CampusId,
                                                                PostedData.Description?.Trim().ToSafeString(),
                                                                PostedData.ChallanMethodId,
                                                                PostedData.WHTaxPolicyId,
                                                                ...
                                                                DateTime.Now,
                                                                Session_Manager.UserId,
                                                                (int?)DOCUMENT_TYPE.ACCOUNT_CLASS_FEE_STRUCTURE,
                                                                (int?)DOCUMENT_STATUS.ACTIVE_ACCOUNT_CLASS_FEE_STRUCTURE_FEE_TYPE,
                                                                true,
