...
namespace office360.Areas.AAccount.HelperCode
{
    ...
    {
        #region HELPER FOR :: CHECK IF ChartOfAccount(ACTIVE_CHARTOFACCOUNT) ALREADY EXIST
        ...
        #region HELPER FOR :: CHECK IF DiscountType(ACTIVE_DISCOUNTTYPE) ALREADY EXIST
        ...
        #region HELPER FOR :: CHECK IF FeeType(ACTIVE_FEETYPE) ALREADY EXIST
        public static int? IS_EXIST_AFTM_FEETYPE_BY_PARAMETER(SQLParamters PostedData)
        {
            using (FASEntities db = new FASEntities())
            {
                bool IsRecordExist = false;
                ...
                try
                {
                    switch (PostedData.OperationType)
                    {
                        ...
                        case nameof(DB_OperationType.UPDATE_DATA_INTO_DB):
                            #region IN CASE OF UPDATE :: CHECK IF ENTERY RECORD EXIST , BASED ON SYSTEM GUID
                            IsRecordExist = db.AFTM_FeeType.Any(x => x.GuID == PostedData.GuID);
                            #endregion
                            if (!IsRecordExist)
                                Response = (int?)Http_DB_Response.CODE_DATA_DOES_NOT_EXIST;
                            else
